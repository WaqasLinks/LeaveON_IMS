...
namespace MYBUSINESS.Controllers
{
    ...
    public class DashboardController : Controller
    {
        ...
        public ActionResult SearchData(string custId, string startDate, string endDate)
        {
            ...
            {
                DateTime PKDate = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, TimeZoneInfo.FindSystemTimeZoneById("Pakistan Standard Time"));
                dtStartDate = new DateTime(PKDate.Year, PKDate.Month, 1);
            ...
            ViewBag.TotalInventory = db.Products.Count();
            ViewBag.TotalRentals = (decimal)(db.SOes.Where(x => x.SelectedOption == 1 && x.Date >= dtStartDate && x.Date <= dtEndtDate).Sum(x => x.SaleOrderAmount) ?? 0);
            ViewBag.TotalSales = (decimal)(db.SOes.Where(x => x.SelectedOption == 2 && x.Date >= dtStartDate && x.Date <= dtEndtDate).Sum(x => x.SaleOrderAmount) ?? 0);
            ViewBag.TotalRepairs = (decimal)(db.SOes.Where(x => x.SelectedOption == 3 && x.Date >= dtStartDate && x.Date <= dtEndtDate).Sum(x => x.SaleOrderAmount) ?? 0);
            ViewBag.TotalTaxes = (decimal)(db.Products.Where(x => x.DateOfPurchase >= dtStartDate && x.DateOfPurchase <= dtEndtDate).Sum(x => x.Tax) ?? 0);
            ViewBag.Customers = db.Customers;
            ...
            {
                return PartialView("_SelectedSOSR");
            }
        ...
        public ActionResult Create([Bind(Prefix = "Customer", Include = "Name,Address")] Customer Customer, [Bind(Prefix = "SaleOrder", Include = "BillAmount,Balance,PrevBalance,BillPaid,Discount,CustomerId,Remarks,Remarks2,PaymentMethod,PaymentDetail,SaleReturn,BankAccountId")] SO sO, [Bind(Prefix = "SaleOrderDetail", Include = "ProductId,SalePrice,Quantity,SaleType,PerPack,IsPack,Product")] List<SOD> sOD, FormCollection collection)

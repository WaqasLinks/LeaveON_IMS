...
namespace MYBUSINESS.Controllers
{
    ...
    public class SOSRController : Controller
    {
        ...
        public ActionResult ToggleSaleStatus(string SOSerial, bool SaleStatus)
        {
            decimal decSOSerial = int.Parse(SOSerial.Split('_').Last());
            SO sO = db.SOes.FirstOrDefault(x => x.SOSerial == decSOSerial);

            sO.SaleReturn = SaleStatus;
            db.Entry(sO).State = EntityState.Modified;
            db.Entry(sO).Property(x => x.SaleReturn).IsModified = true;

            foreach (SOD sod in sO.SODs)
            {
                sod.SaleType = SaleStatus;
                db.Entry(sod).State = EntityState.Modified;
                db.Entry(sod).Property(x => x.SaleType).IsModified = true;
            }
            db.SaveChanges();
        ...
        [ValidateInput(false)]
        public ActionResult Create([Bind(Prefix = "Customer", Include = "Name,Address")] Customer Customer, [Bind(Prefix = "SaleOrder", Include = "BillAmount,Balance,PrevBalance,BillPaid,Discount,CustomerId,Remarks,Remarks2,PaymentMethod,PaymentDetail,SaleReturn,BankAccountId,Status,AssignedTo,RentStartDate,RentEndDate,RentStartTime,RentEndTime,SelectedOption")] SO sO, [Bind(Prefix = "SaleOrderDetail", Include = "ProductId,SalePrice,Quantity,SaleType,PerPack,IsPack,Product")] List<SOD> sOD, FormCollection collection)

        {
            ...
            if (ModelState.IsValid)
            {
                ...
                sO.Profit = 0;
        //Employee emp = (Employee)Session["CurrentUser"];//S@S
        //sO.EmployeeId = emp.Id;//S@S
      ...
      if (FileTempered(viewer.LocalReport.ReportPath) == true)
        ...
        bytes = viewer.LocalReport.Render("PDF", deviceInfo, out mimeType, out encoding, out fileNameExtension, out streams, out warnings);
        ...
        public ActionResult Edit(string id, bool update)
        {
            ...
            List<TextValuePair> myOptionLst = new List<TextValuePair> {
                            new TextValuePair {
                                Text = "Order",
                                Value = "false"
                            },
            ...
            ////////////////
            List<TextValuePair> myPaymentOptionLst = new List<TextValuePair> {
                            new TextValuePair {
                                Text = "Cash",
                                Value = "Cash"
                            },
            ...
            ViewBag.BankAccounts = new SelectList(db.BankAccounts, "Id", "Name");
            if (sO.SelectedOption == 1) ViewBag.Rental = true;
            if (sO.SelectedOption == 2) ViewBag.Sale = true;
            if (sO.SelectedOption == 3) ViewBag.Repair = true;
        ...
        public ActionResult Edit(SaleOrderViewModel saleOrderViewModel1)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                sO.Remarks2 = newSO.Remarks2;//
                sO.PaymentMethod = newSO.PaymentMethod;
                sO.PaymentDetail = newSO.PaymentDetail;
                sO.BankAccountId = newSO.BankAccountId;
                sO.Status = newSO.Status;
                sO.AssignedTo = newSO.AssignedTo;
                sO.RentStartDate = newSO.RentStartDate;
                sO.RentEndDate = newSO.RentEndDate;
                sO.RentStartTime = newSO.RentStartTime;
                sO.RentEndTime = newSO.RentEndTime;
                sO.SelectedOption = newSO.SelectedOption;

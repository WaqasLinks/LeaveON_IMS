...
namespace MYBUSINESS.Controllers
{
    public class POPRController : Controller
    {
        ...
        public ActionResult Create(string IsReturn)
        {
            ...
            purchaseOrderViewModel.Products = db.Products.Where(x => x.Saleable == true && x.IsService==false);
            ...
            Supplier defaultSupplier = db.Suppliers.FirstOrDefault(x => x.Id == 0);//db.Suppliers.FirstOrDefault(x => x.IsCreditor == false);
      ...
      byte[] bytes = viewer.LocalReport.Render("PDF", deviceInfo, out mimeType, out encoding, out fileNameExtension, out streams, out warnings);//, null, out mimeType, out encoding, out extension, out streamIds, out warnings);
